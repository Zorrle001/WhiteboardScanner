function loadPushShare() {
    ...
    if (pushSharingEnabled) {
        if (!window.subscription) {
            ...
            fetch("https://api.whiteboardscanner.zorrle001.dev/activation", {
                method: "post",
                headers: {
        ...
        if (pushSharingCheckbox.checked) {
            ...
            Notification.requestPermission().then((result) => {
                if (result === "granted") {
                    ...
                    fetch(
                        "https://api.whiteboardscanner.zorrle001.dev/activation",
                        {
                            method: "post",
                            headers: {
                                "Content-type": "application/json",
                    ...
                    fetch(
                        "https://api.whiteboardscanner.zorrle001.dev/send_notification",
                        {
                            method: "post",
                            headers: {
                                "Content-type": "application/json",
                            ...
                            body: JSON.stringify({
                                subscription: window.subscription,
                                payload:
                                    "Neues Gerät für Push-Share registriert",
                                ttl: 60 * 3,
                            }),
                        }
                    );
